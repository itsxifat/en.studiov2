  ...
  if (projects.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center p-8 bg-neutral-900/50 border border-neutral-700 rounded-lg">
        ...
        <p className="text-neutral-400">Please <Link href="/admin/photography/projects" className="text-[#53A4DB] hover:underline">create a project</Link> first to manage its photos.</p>
      </div>
    );
  }
      ...
      <AnimatePresence>
        {projects.map(project => (
          ...
          >
            ...
            <div className="p-4">
              <h3 className="text-lg font-bold text-white group-hover:text-[#53A4DB] transition-colors">{project.title}</h3>
...
const ProjectPhotoGrid = ({ project, onBack, onEdit, onDelete, photos, isLoading, error }) => {
  return (
    <div>
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-sm font-semibold text-neutral-300 hover:text-[#53A4DB] transition-colors mb-6"
      >
        <ArrowLeft size={16} />
        Back to Projects
      </button>
      <h2 className="text-3xl font-bold text-white mb-8">Managing: <span className="text-[#53A4DB]">{project.title}</span></h2>

      {isLoading && <div className="flex justify-center items-center min-h-[50vh]"><Loader2 size={40} className="animate-spin text-[#53A4DB]" /></div>}
      ...
      {!isLoading && !error && photos.length === 0 && (
         <div className="flex flex-col items-center justify-center min-h-[30vh] text-center p-8 bg-neutral-900/50 border border-neutral-700 rounded-lg">
          ...
          <p className="text-neutral-400">This project is empty. <Link href="/admin/photography/upload" className="text-[#53A4DB] hover:underline">Upload photos</Link> to see them here.</p>
        </div>
      )}
          ...
          <AnimatePresence>
            {photos.map(photo => (
              ...
              >
                ...
                <div className="p-4">
                  <p className="text-sm font-bold text-[#53A4DB] truncate" title={photo.title}>{photo.title || "Untitled"}</p>
                  <p className="text-xs text-neutral-400 truncate mt-1" title={photo.description}>{photo.description || "No description"}</p>
                  <div className="flex gap-2 mt-4">
                    <button
                      onClick={() => onEdit(photo)}
                      className="w-full inline-flex items-center justify-center gap-2 text-sm font-semibold text-[#53A4DB] bg-cyan-900/30 hover:bg-cyan-900/60 h-10 px-4 rounded-md transition-colors border border-cyan-800/50"
                    >
                      <Edit size={14} />
                      Edit
                    </button>
...
export default function ManagePhotographyPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <h1 className="text-4xl md:text-5xl font-bold tracking-tight mb-10 font-heading uppercase text-[#53A4DB] flex items-center gap-4">
        <Camera size={44} />
        Manage Photography
      </h1>
